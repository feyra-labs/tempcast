...
1. поточечные и оконные (``check_codes``) - одинаковы для офлайн-сборки кэша
   (``qc_station``) и для окна истории после аугментаций (``qc_window``);
2. станционные (``station_checks``) - выполняются один раз при сборке кэша;
3. отбор станций (``station_selection``) - по длине ряда, доле валидной температуры
...
QC_VERSION = "3.0" # Увеличивать при изменении; входит в ключ кэша.
CHANNELS = ("T", "P", "RH")
...
def check_changepoint(T, ok, t0, cfg=DEFAULT_QC):
    ...
    «гармоники + ступенька в найденной точке». Разладка значима, если |m₂ − m₁| ≥ cp_min_shift и z ≥ cp_min_z,
