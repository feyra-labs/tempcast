...
Сборка:  исходные файлы станций → QC → климатология → кэш на диске.
...
from mayak.data.qc import CHANNELS, QC_VERSION, QCCode, qc_station
...
def read_source(path):
    """Исходный файл станции → dict(T, P, RH, valid, t0)."""
...
def key_payload(manifest, rows=None):
    rows = read_manifest(manifest) if rows is None else rows
    stations = sorted((r["id"], file_sha256(source_path(manifest, r["id"]))) for r in rows)
    return {
        "format": CACHE_FORMAT, "qc": QC_VERSION, "calendar": CALENDAR_VERSION,
...
def process_station(path):
    """QC + климатология одной станции. Вызывается в пуле процессов."""
...
def _qc_fractions(codes):
    ...
    for j, ch in enumerate(CHANNELS):
        for c in QCCode:
            if c:
                out[f"{ch}/{c.name}"] = float(np.mean((codes[:, j] & c) > 0)) if len(codes) else 0.0
    return out


def build_cache(manifest, cache_root=None, jobs=1, force=False):
    rows = read_manifest(manifest)
    t_start = time.perf_counter()
    payload = key_payload(manifest, rows)
    ...
    paths = [source_path(manifest, r["id"]) for r in rows]
    if jobs > 1:
        with ProcessPoolExecutor(jobs) as ex:
            results = list(ex.map(process_station, paths, chunksize=4))
    else:
    ...
    with open(os.path.join(tmp, "meta.json"), "w") as f:
        json.dump(dict(key=key, payload=payload, excluded=excluded,
                       qc_total=_qc_fractions(all_codes),
                       build_seconds=time.perf_counter() - t_start), f, indent=1, ensure_ascii=False)
    with open(os.path.join(tmp, "qc_report.csv"), "w", newline="") as f:
